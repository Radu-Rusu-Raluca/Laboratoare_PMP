...
d = stats.gamma.rvs(5, scale=1.0/2, size=10000)

x = [0 for i in range(10000)]
for i in range(10000):
    if i%100<25:
        x[i]=a[i] + stats.expon.rvs(0.1/4)
    elif i%100>=25 & i%100<50:
        x[i]=b[i] + stats.expon.rvs(0.1/4)
    elif i%100>=50 & i%100<80:
        x[i] = c[i] + stats.expon.rvs(0.1/4)
    else:
        x[i] = d[i] + stats.expon.rvs(0.1/4)

...
with pm.Model() as model_g:
    α = pm.Normal('α', mu=statistics.mean(mom_age), sd=np.std(mom_age))
    β = pm.Normal('β', mu=statistics.mean(ppvt), sd=np.std(ppvt))
    ε = pm.HalfCauchy('ε', 5)
    μ = pm.Deterministic('μ', α + β * mom_age_std)
    y_pred = pm.Normal('y_pred', mu=μ, sd=ε, observed=ppvt_std)
